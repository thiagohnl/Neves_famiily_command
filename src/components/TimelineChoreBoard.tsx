        ...
        {(() => {
          ...
          const renderTimeline = (members: FamilyMember[]) => (
            <div className="flex">
              ...
              <div className="flex-1 min-w-0">
                {/* Headers (hidden on mobile since we have the selector) */}
                <div className="relative z-10 bg-white grid" style={{ gridTemplateColumns: `repeat(${members.length}, 1fr)` }}>
                  {members.map(member => (
                ...
                <div className="relative mt-4 sm:mt-4">
                  ...
                  <div className="grid relative z-10" style={{ gridTemplateColumns: `repeat(${members.length}, 1fr)` }}>
                    {members.map(member => (
          ...
          return (
            <>
              ...
              <div className="sm:hidden">{renderTimeline(mobileMembers)}</div>
              {/* Desktop: all members */}
              <div className="hidden sm:block">{renderTimeline(desktopMembers)}</div>
            </>
          );
        })()}
      </motion.div>
